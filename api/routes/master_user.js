...
router.post('/master/addUser',async  (req,res,next) => {
    ...
    try{
        ...
        const existingUser = await MasterUser.findOne({gmail: req.body.gmail});
 
        if(existingUser){
            return res.status(400).json({ status:false, message: 'Gmail must be unique' });
        }
      ...
      masterUser.save().then(
          ...
          res.status(200).json({
              status:true,
              msg: result.gmail + " Register Successfully"+result._id,
              data:[
                  {
                      id: result._id
                  }
